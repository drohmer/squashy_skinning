...
{
#ifdef SOLUTION
    const float r = gui_scene.falloff;
    ...
    for(int k=0; k<N; ++k)
    {
        ...
        if(dist<3.5*r) { // speed-up computation

            // Weight with respect to distance
            const float w = std::exp( -dist*dist/(r*r) );

            // Current position
            vec3& p = shape.position[k];
            const vec3& p0 = position_saved[k];
            const vec3& center = picking.p0;


            if(gui_scene.deformer_type==deform_translate && gui_scene.deformer_direction==camera_direction) {
                const vec3 translation = camera.orientation * vec3(t.x, t.y, 0);
                p = p0 + w * translation;
            }

            if(gui_scene.deformer_type==deform_translate && gui_scene.deformer_direction==normal_direction) {
                const vec3 translation = picking.n0 * t.y;
                p = p0 + w * translation;
            }

            if(gui_scene.deformer_type==deform_twist && gui_scene.deformer_direction==camera_direction) {
                const float angle = - w * t.x * 2 * 3.14f;
                const vec3 camera_normal = camera.orientation.col(2);
                const mat3 R = rotation_from_axis_angle_mat3(camera_normal, angle);
                p = center + R*(p0-center);
            }

            if(gui_scene.deformer_type==deform_twist && gui_scene.deformer_direction==normal_direction) {
                const float angle = - w * t.x * 2 * 3.14f;
                const mat3 R = rotation_from_axis_angle_mat3(picking.n0, angle);
                p = center + R*(p0-center);
            }

            if(gui_scene.deformer_type==deform_scale) {
                p = center + (1+t.x*w)*(p0-center);
            }
...
void scene_model::mouse_click(scene_structure& , GLFWwindow* window, int , int , int )
{
    ...
    const bool mouse_release_left  = glfw_mouse_released_left(window);
...
{
    const std::string filename = "scenes/sources/local_deformers/assets/face.obj";
